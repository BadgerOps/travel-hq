...
describe("import review routes", () => {
  ...
  it("matches a repeated confirmation to its existing booking and enriches details", async () => {
    const trip = await new TripRepo(env.DB, ctx).create({
      title: "Glacier",
      startsOn: "2026-08-05",
      endsOn: "2026-08-09",
    ...
    const createDraft = async (
      ...
      const email = await InboundEmailRepo.forIngest(env.DB, "hh-a").create({
        ...
        to: "trips@example.com",
        subject: "Fwd: KOA Confirmation #21081900",
        messageId,
        raw: "forwarded reservation",
      });
    ...
    const first = await createDraft("<first@example.com>", {
    ...
    const repeated = await createDraft("<forwarded-again@example.com>", {
      propertyName: "St. Mary / East Glacier KOA Holiday",
      nights: 4,
      total: 504,
      campsite: "Pull Thru, 50/30 Amps, Full Hookups",
      specialRequest: "Two neighboring sites",
    });
    expect((await postJson(appAs(), "/api/imports/accept", {
      draftIds: [repeated.id],
      tripId: trip.id,
    })).status).toBe(200);
    ...
    const booking = await env.DB.prepare(
      "SELECT id, details FROM booking WHERE trip_id = ?",
    ).bind(trip.id).first<{ id: string; details: string }>();
    expect(JSON.parse(booking!.details)).toMatchObject({
      total: "$549.24",
      campsite: "Pull Thru, 50/30 Amps, Full Hookups",
      specialRequest: "Two neighboring sites",
    });
    expect(
      await env.DB.prepare(
        "SELECT booking_id FROM draft_booking WHERE id = ?",
      ).bind(repeated.id).first(),
    ).toEqual({ booking_id: booking!.id });
  });
