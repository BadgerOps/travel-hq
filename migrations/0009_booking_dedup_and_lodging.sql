...
CREATE TABLE _booking_duplicate_map_0009 (
  duplicate_id TEXT PRIMARY KEY,
  keeper_id    TEXT NOT NULL
);

INSERT INTO _booking_duplicate_map_0009 (duplicate_id, keeper_id)
...
UPDATE booking AS keeper
   SET details = (
     SELECT json_patch(duplicate.details, keeper.details)
       FROM _booking_duplicate_map_0009 map
 ...
 WHERE keeper.id IN (
   SELECT keeper_id FROM _booking_duplicate_map_0009
 );
...
UPDATE draft_booking
 ...
 WHERE booking_id IN (
   SELECT duplicate_id FROM _booking_duplicate_map_0009
 );
...
DROP TABLE _booking_duplicate_map_0009;
